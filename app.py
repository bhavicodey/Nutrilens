...
#  Main Tabs Setup
# ---------------------------
tab_scan, tab_ingredients, tab_stats, tab_leaderboard  = st.tabs(
    ["🏠 Food Scan", "⚠️ Ingredient Scanner", "📈 Your Trends", "🏆 Leaderboard" ]
)
with tab_stats:
    if "user" not in st.session_state:
        st.warning("You need to log in to access your trends 📈")
    else:
...
with tab_leaderboard:
    if "user" not in st.session_state:
        st.warning("You need to log in to access the leaderboard 🏆")
    else:
        show_leaderboard()
